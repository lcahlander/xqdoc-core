...
:
	(
		(
			...
			|
			( "declare" "default" "element" )  => setter 
			|
			( "declare" "default" "function" )  => setter 
			|
			( "declare" "base-uri" ) =>  baseUriDecl 
			|
			( xqdocComment "declare" "variable" ) => varFunctionDecl 
			|
			( "declare" "variable" ) => varFunctionDecl 
			|
			( "declare" "validation" ) => "declare" "validation" schemaMode 		
			|
			( "import" "schema" ( ("namespace" NCNAME EQ) | ("default" "element" "namespace") )? STRING_LITERAL ( "at" STRING_LITERAL )?)
			|
			( xqdocComment "declare"  "function" ) => varFunctionDecl
			|
			( "declare"  "function" ) => varFunctionDecl
			|
			( "declare" "option" ) => existOptionDecl
		)
		separator	
	)*
	;	
...
versionDecl
:
	"xquery" "version" STRING_LITERAL separator
	;
	
varFunctionDecl
:
	(
		( (xqdocComment)? "declare" "function" ) => functionDecl
		|
...
:
	"declare" "option" name=qName STRING_LITERAL
	;
...
:
	(xqdocComment)? "declare" "variable" DOLLAR varName=qName( typeDeclaration )? ( (LCURLY expr RCURLY) | "external" ) 
	{ 
...
valueExpr	
:
	("validate") => validateExpr | pathExpr
	;
...
validateExpr
:
	"validate" ( ("global") | ("context" schemaContextLoc) | (schemaMode (schemaContext)? ))? LCURLY expr RCURLY
	;
...
/*
 * This production is slightly different from the BNF, as it doesn't list
 * a computed constructor as a valid content. This is handled via predicates 
 * with the specific text strings in the primary expression rule.
 */
constructor 
:
	dirElemConstructor
...
:
	"namespace" tmpStr=ncnameOrKeyword LCURLY expr RCURLY
	;
...
elementTest 
: 
	"element" LPAREN ( elementNameOrWildcard (COMMA typeNameOrWildcard ("nillable")?)? )? RPAREN ;


attributeTest 
: 
	"attribute" LPAREN ( attributeNameOrWildcard (COMMA typeNameOrWildcard)? ) ? RPAREN 
	;
...
:
	(str1=qName | schemaGlobalTypeName) SLASH ( str1=qName SLASH )*
	;
...
:
	("type" LPAREN) => schemaGlobalTypeName
	|
	( (schemaContextPath)? str1=qName)
	;
