...
expr 
:
	exprSingle ( BANG exprSingle )? ( COMMA exprSingle ( BANG exprSingle )? )*
	;
...
:
	DOLLAR name=qName ( typeDeclaration )? COLON EQ exprSingle ( BANG exprSingle )?
	;
...
ifExpr 
: 
	"if" LPAREN expr RPAREN "then" exprSingle ( BANG exprSingle )? "else" exprSingle ( BANG exprSingle )? 
	;
...
stepExpr 
:
	( ( "text" | "node" | "element" | "attribute" | "comment" | "processing-instruction" | "document-node"  | "schema-attribute" | "schema-element") LPAREN ) => axisStep
	|
	( ( "element" | "attribute" | "text" | "document" | "processing-instruction" | "comment" ) LCURLY ) => filterExpr
	|
